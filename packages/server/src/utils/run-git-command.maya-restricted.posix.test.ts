...
describe.skipIf(process.platform === "win32")("Maya restricted Git read policy", () => {
  test("does not execute repository fsmonitor or external/textconv diff helpers", async () => {
    ...
    writeFileSync(join(root, ".gitattributes"), "tracked.txt diff=evil\n");
    ...
    try {
      await runGitCommand(["status", "--porcelain"], { cwd: authority.rootAccessPath });
      await runGitCommand(["diff"], { cwd: authority.rootAccessPath });
      await expect(
        runGitCommand(["diff", "--textconv"], { cwd: authority.rootAccessPath }),
